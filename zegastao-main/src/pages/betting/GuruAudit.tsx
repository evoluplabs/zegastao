  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <h3 className="text-lg font-bold text-slate-100">Desmascarador de Guru</h3>
        <p className="text-sm text-slate-400">Recebeu um bilhete "premiado" de algum tipster? Manda o print que o Zé mostra a real.</p>
      </div>
      ...
      {result && (
        <div className="space-y-3 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
          ...
          <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3">
            ...
            <p className="mt-1 text-sm text-slate-300">{verdict}</p>
          </div>
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="rounded-xl bg-slate-800/60 p-2">
      <div className={highlight ? 'text-xl font-extrabold text-amber-400' : 'text-xl font-bold text-slate-100'}>{value}</div>
