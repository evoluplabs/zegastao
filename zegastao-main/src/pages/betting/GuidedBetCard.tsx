...
const STATUS_STYLE: Record<string, { icon: typeof TrendingUp; cls: string }> = {
  green: { icon: CheckCircle2, cls: 'text-emerald-400 border-emerald-500/30 bg-emerald-500/10' },
  ...
  if (card.skip) {
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 text-center">
        <p className="text-slate-300">{card.reasoning}</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/70">
      ...
      <div className="space-y-4 p-4">
        {/* Por que essa aposta */}
        <div className="flex items-start gap-2 rounded-xl bg-slate-800/40 p-3 text-sm text-slate-300">
          <Info className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" /> {card.reasoning}
        </div>
        ...
        {card.needsSeal && !sealAccepted && (
          <div className="space-y-3 rounded-xl border border-amber-500/40 bg-amber-500/10 p-4">
            ...
            <div className="flex gap-2">
              <Button onClick={() => setSealAccepted(true)} className="flex-1 bg-amber-500 text-slate-950 hover:bg-amber-400">Topo, é diversão 🎲</Button>
              <button onClick={onUpdated} className="flex-1 rounded-lg border border-slate-600 text-sm text-slate-300">Me mostra algo mais seguro</button>
            </div>
          </div>
        )}
          ...
          <>
            ...
            <div className="space-y-3">
              {card.legs.map((leg) => {
                ...
                return (
                  <div key={key} className="rounded-xl border border-slate-800 bg-slate-800/30 p-3">
                    <div className="mb-1 flex items-center justify-between">
                      <span className="text-xs text-slate-500">{leg.league} · {leg.marketLabel}</span>
                      <span className="text-xs text-slate-500">prob {leg.modelProbPct}%</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-slate-100">{leg.homeTeam} x {leg.awayTeam}</p>
                        <p className="text-sm text-emerald-400">{leg.selection}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs text-slate-500">só vale ≥ {leg.minOdd}</p>
                        <div className="mt-1 flex items-center gap-1">
                          <span className="text-xs text-slate-400">odd:</span>
                          <input
                            type="number" step="0.01" inputMode="decimal"
                            value={odds[key]} onChange={(e) => setOdds((p) => ({ ...p, [key]: e.target.value }))}
                            className="h-8 w-16 rounded-md border border-slate-700 bg-slate-900 px-2 text-right text-sm text-slate-100 focus:border-emerald-400 focus:outline-none"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Passo a passo */}
                    <ol className="mt-2 space-y-0.5 text-[11px] text-slate-500">
            ...
            <Button onClick={recalcNow} loading={loading} variant="outline" className="w-full border-slate-700 text-slate-200">
            ...
            <div className="flex items-center justify-between rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3">
              <div>
                <p className="text-xs text-slate-400">Apostar</p>
                <p className="text-xl font-extrabold text-emerald-400">{formatBRL(stake)}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-slate-400">Pode pagar</p>
                <p className="text-lg font-bold text-slate-100">+{formatBRL(potential)}</p>
              </div>
            </div>
