  ...
  return (
    <div className="mx-auto max-w-md space-y-6 p-4">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-foreground">Configuração do Raid</h2>
        <p className="text-sm text-muted-foreground">Configure seu raid uma vez. Depois o Zé trabalha por você.</p>
      </div>
            ...
            <div className="flex flex-col items-center gap-1">
              <div className={cn('flex h-8 w-8 items-center justify-center rounded-full border-2 text-xs',
                i < step ? 'border-green-500 bg-green-500 text-stone-950'
                  : i === step ? 'border-green-400 text-green-400' : 'border-border text-muted-foreground/70')}>
                {i < step ? <Check className="h-4 w-4" /> : i + 1}
              </div>
              <span className={cn('text-[9px]', i === step ? 'text-green-400' : 'text-muted-foreground/70')}>{s}</span>
            </div>
            {i < STEPS.length - 1 && <div className={cn('mx-1 h-0.5 flex-1', i < step ? 'bg-green-500' : 'bg-border')} />}
          </div>
        ))}
      </div>

      <div className="min-h-56 rounded-2xl border border-border bg-card/60 p-5">
        {step === 0 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-foreground"><Wallet className="h-5 w-5 text-green-400" /><h3 className="text-lg font-bold">Banca do Raid</h3></div>
            <p className="text-sm text-muted-foreground">Quanto você coloca neste raid? É o teto — o Zé nunca passa disso.</p>
            ...
            <div className="flex justify-between text-xs text-muted-foreground/70"><span>R$ 2</span><span>R$ 100</span></div>
            ...
            {suggestion && <p className="rounded-lg bg-secondary/60 px-3 py-2 text-[11px] leading-relaxed text-muted-foreground">{suggestion}</p>}
          </div>
        )}

        {step === 1 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-foreground"><Target className="h-5 w-5 text-green-400" /><h3 className="text-lg font-bold">Objetivo do Raid</h3></div>
            <p className="text-sm text-muted-foreground">Quanto você quer fazer a banca crescer antes de encerrar?</p>
            {/* Alterna entre meta em % e meta em R$ */}
            <div className="flex rounded-xl border border-border p-0.5 text-xs">
              <button onClick={() => setGoalMode('pct')} className={cn('flex-1 rounded-lg py-1.5 font-semibold', goalMode === 'pct' ? 'bg-green-500 text-stone-950' : 'text-muted-foreground')}>Em %</button>
              <button onClick={() => { setGoalMode('amount'); if (goalAmount <= budget) setGoalAmount(Math.round(budget * 5)); }} className={cn('flex-1 rounded-lg py-1.5 font-semibold', goalMode === 'amount' ? 'bg-green-500 text-stone-950' : 'text-muted-foreground')}>Em R$</button>
            </div>
            {goalMode === 'pct' ? (
              <div className="flex gap-2 flex-wrap">
                {[20, 50, 100, 200].map((v) => (
                  <button key={v} onClick={() => setGrowth(v)} className={cn('rounded-full border px-3 py-1.5 text-sm', growth === v ? 'border-green-400 bg-green-400/15 text-green-300' : 'border-border text-foreground/80')}>+{v}%</button>
                ))}
              </div>
            ) : (
              <div className="space-y-2">
                <div className="flex gap-2 flex-wrap">
                  {amountPresets.map((v) => (
                    <button key={v} onClick={() => setGoalAmount(v)} className={cn('rounded-full border px-3 py-1.5 text-sm', goalAmount === v ? 'border-green-400 bg-green-400/15 text-green-300' : 'border-border text-foreground/80')}>{formatBRL(v)}</button>
                  ))}
                </div>
                <input type="number" min={budget + 1} value={goalAmount || ''} onChange={(e) => setGoalAmount(Number(e.target.value))} placeholder={`Ex: ${formatBRL(Math.round(budget * 10))}`} className="h-10 w-full rounded-xl border border-border bg-secondary px-3 text-foreground placeholder:text-muted-foreground/50 focus:border-green-400 focus:outline-none" />
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-foreground"><Trophy className="h-5 w-5 text-green-400" /><h3 className="text-lg font-bold">Escolha a Arena</h3></div>
            <div className="flex flex-wrap gap-2">
              {BETTING_LEAGUES.map((l) => (
                <button key={l.key} onClick={() => toggleLeague(l.key)} className={cn('rounded-full border px-3 py-1.5 text-sm', leagues.includes(l.key) ? 'border-green-400 bg-green-400/15 text-green-300' : 'border-border text-foreground/80')}>{l.label}</button>
              ))}
            </div>
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground">Times de preferência (opcional, separe por vírgula)</p>
              <input value={teamsText} onChange={(e) => setTeamsText(e.target.value)} placeholder="Ex: Brasil, Argentina" className="h-10 w-full rounded-xl border border-border bg-secondary px-3 text-foreground placeholder:text-muted-foreground/50 focus:border-green-400 focus:outline-none" />
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-3">
            <h3 className="text-lg font-bold text-foreground">Dificuldade do Raid</h3>
            <p className="text-sm text-muted-foreground">Você sempre confirma antes de apostar. Isso define o estilo dos encontros.</p>
            {ZE_RISK_LEVELS.map((r) => (
              <button key={r.level} onClick={() => setRiskLevel(r.level)} className={cn('w-full rounded-xl border p-3 text-left transition-colors', riskLevel === r.level ? 'border-green-400 bg-green-400/10' : 'border-border hover:border-border')}>
                <div className="flex items-center gap-2 font-semibold text-foreground">{r.emoji} {r.label}</div>
        ...
        {step === 4 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-green-400" /><h3 className="text-lg font-bold text-foreground">⚔️ Pacto do Aventureiro</h3></div>
            {CLAUSES.map((clause, i) => (
              <label key={i} className={cn('flex items-start gap-3 rounded-xl border p-3 cursor-pointer', accepted[i] ? 'border-green-500/40 bg-green-500/10' : 'border-border')}>
                ...
                <span className="text-sm leading-relaxed text-foreground/80">{clause}</span>
              </label>
            ))}
      ...
      <div className="flex items-center justify-between gap-3">
        <button onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0} className="flex items-center gap-1 rounded-xl border border-border px-4 py-2.5 text-sm text-foreground/80 disabled:opacity-30"><ChevronLeft className="h-4 w-4" /> Voltar</button>
        {step < STEPS.length - 1 ? (
