...
const CASES = [
  {
    ...
    resultIcon: '💰',
    resultColor: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400',
  },
  {
    avatar: '🛡️',
    ...
    <section className="border-b border-[#2a2d3e] py-16 md:py-24 bg-[#0f1117]">
      <div className="mx-auto max-w-4xl px-4">
        <div className="text-center mb-12 reveal">
          <p className="text-xs font-bold uppercase tracking-widest text-emerald-400 mb-3">— Batalhas Vencidas —</p>
        ...
        <div className="reveal-scale">
          <div className="rounded-2xl border border-[#2a2d3e] bg-[#1a1d27] p-8 shadow-xl shadow-black/30">
          ...
          <div className="flex items-center justify-center gap-4 mt-6">
            <button
              ...
              className="h-9 w-9 rounded-full border border-[#2a2d3e] bg-[#1a1d27] flex items-center justify-center hover:bg-[#1e2235] transition-colors text-slate-400"
            >
            ...
            <div className="flex gap-2">
              {CASES.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setIdx(i)}
                  className={`h-2 rounded-full transition-all ${i === idx ? 'bg-emerald-400 w-6' : 'bg-slate-700 w-2'}`}
                />
              ))}
            </div>
            <button
